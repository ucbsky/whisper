/// Copied from https://github.com/ucbsky/elsa
///
...
impl IdGen {
    ...
    /// Any two [`IdGen`] instantiated with different tags will never overlap message ids, assuming that they
    /// use less than `send_bound` / `recv_bound` messages.
    pub fn new_tagged(tag: u64, send_bound: u64, recv_bound: u64) -> Self {
        Self {
            next_send_id: COMMON_MESSAGE_ID_START + send_bound * tag,
            next_recv_id: COMMON_MESSAGE_ID_START + recv_bound * tag,
            next_send_id_bound: COMMON_MESSAGE_ID_START + send_bound * (tag + 1),
            next_recv_id_bound: COMMON_MESSAGE_ID_START + recv_bound * (tag + 1),
        }
    }

    pub fn next_send_id(&mut self) -> SendId {
        if self.next_send_id == self.next_send_id_bound {
            panic!("sending more messages than expected")
        }
