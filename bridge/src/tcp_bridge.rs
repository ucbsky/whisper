...
use crate::id_tracker::{ExchangeId, RecvId, SendId, REGISTER_MESSAGE_ID};
...
impl TcpConnection {
    fn new(socket: TcpStream, uid: ClientID) -> Self {
        ...
        // write loop
        {
            ...
            tokio::spawn(async move {
                let mut write_socket =
                    BufWriter::with_capacity(CLIENT_TCP_BUFFER_SIZE, write_socket);
...
pub async fn localhost_pair(port: u16) -> (TcpConnection, TcpConnection) {
    let server_handle = tokio::spawn(async move {
        ...
        TcpConnection::new_server_side(socket).await
    });
