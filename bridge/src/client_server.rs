...
impl ClientsPool {
    ...
    /// Subscribe and get message that does not contain any references
    pub async fn subscribe_and_get_stream<T: Communicate>(
        &self,
        message_id: RecvId,
    ) -> Result<Vec<T::Deserialized>> {
        // for each client, subscribe the struct
...
/// returns a vector of length `batch_size` with each element a pair of
/// (address_to_server0, address_to_server1)
pub async fn batch_meta_clients(
    batch_size: usize,
    batch_id: usize,
    ...
    let mut progresses = Vec::with_capacity(batch_size * 2);
    // println!("{} {}", batch_id, batch_size);
    for uid in (batch_id * batch_size)..(batch_size * (batch_id + 1)) {
        // println!("{}", uid);
