...
mod tests {
    ...
    fn tweaks(tweak: Tweak) {
        ...
        let mut server1 = Server::<Field64>::new(dim, true).unwrap();
        ...
        let mut share1_field = Field64::byte_slice_into_vec(&share1_original).unwrap();
        let unpacked_share1 = unpack_proof_mut(&mut share1_field, dim).unwrap();

        let one = Field64::from(1);
        ...
        let share1_modified = Field64::slice_into_byte_vec(&share1_field);
