...
use sha3::{
    ...
    Shake128, Shake128Core, Shake128Reader, Shake256, Shake256Core, Shake256Reader,
};
#[cfg(feature = "crypto-dependencies")]
...
/// The XOF based on SHA-3 as specified in [[draft-irtf-cfrg-vdaf-07]] with 256-bit seed.
///
/// [draft-irtf-cfrg-vdaf-07]: https://datatracker.ietf.org/doc/draft-irtf-cfrg-vdaf/07/
#[derive(Clone, Debug)]
pub struct XofShake256(Shake256);

impl Xof<32> for XofShake256 {
    type SeedStream = SeedStreamSha3256;

    fn init(seed_bytes: &[u8; 32], dst: &[u8]) -> Self {
        let mut xof = Self(Shake256::from_core(Shake256Core::default()));
        Update::update(
            &mut xof.0,
            &[dst.len().try_into().expect("dst must be at most 255 bytes")],
        );
    ...
    fn into_seed_stream(self) -> SeedStreamSha3256 {
        SeedStreamSha3256::new(self.0.finalize_xof())
    }
}
...
/// The key stream produced by the cSHAKE256 XOF.
pub struct SeedStreamSha3256(Shake256Reader);

impl SeedStreamSha3256 {
    pub(crate) fn new(reader: Shake256Reader) -> Self {
        Self(reader)
    }
}

impl RngCore for SeedStreamSha3256 {
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        XofReader::read(&mut self.0, dest);
    }
...
/// A `rand`-compatible interface to construct XofShake256 seed streams, with the domain separation tag
/// and binder string both fixed as the empty string.
impl SeedableRng for SeedStreamSha3256 {
    type Seed = [u8; 32];

    fn from_seed(seed: Self::Seed) -> Self {
        XofShake256::init(&seed, b"").into_seed_stream()
    }
}
