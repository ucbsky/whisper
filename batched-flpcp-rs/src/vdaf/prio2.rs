...
use crate::{
    ...
    field::{
        decode_fieldvec, FftFriendlyFieldElement, Field64, FieldElement, FieldElementWithInteger,
    },
    prng::Prng,
    vdaf::{
        prio2::{
...
impl Prio2 {
    ...
    pub fn new(input_len: usize) -> Result<Self, VdafError> {
        ...
        if let Ok(size) = u64::try_from(2 * n) {
            if size > Field64::generator_order() {
    ...
    pub fn prepare_init_with_query_rand(
        &self,
        query_rand: Field64,
        input_share: &Share<Field64, 32>,
        is_leader: bool,
    ) -> Result<(Prio2PrepareState, Prio2PrepareShare), VdafError> {
        let expanded_data: Option<Vec<Field64>> = match input_share {
            Share::Leader(_) => None,
    ...
    pub(crate) fn choose_eval_at<S>(&self, prng: &mut Prng<Field64, S>) -> Field64
    where
        S: RngCore,
    {
        ...
        loop {
            let eval_at: Field64 = prng.get();
            // Unwrap safety: the constructor checks that this conversion succeeds.
            if eval_at.pow(u64::try_from(proof_length).unwrap()) != Field64::one() {
                return eval_at;
            }
        }
    }
...
impl Vdaf for Prio2 {
    ...
    type PublicShare = ();
    type InputShare = Share<Field64, 32>;
    type OutputShare = OutputShare<Field64>;
    type AggregateShare = AggregateShare<Field64>;

    type PrepareState = Prio2PrepareState;
    // Dummies
    type Blinds = u16;
    type PublicProof = u16;
    type BatchedOutputShare = u16;
    type ProofShare = u16;
...
#[derive(Clone, Debug)]
pub struct Prio2PrepareState(Share<Field64, 32>);
...
#[derive(Clone, Debug)]
pub struct Prio2PrepareShare(v2_server::VerificationMessage<Field64>);
...
impl ParameterizedDecode<Prio2PrepareState> for Prio2PrepareShare {
    fn decode_with_param(
        ...
        Ok(Self(v2_server::VerificationMessage {
            f_r: Field64::decode(bytes)?,
            g_r: Field64::decode(bytes)?,
            h_r: Field64::decode(bytes)?,
        }))
    }
}
...
impl Aggregator<32, 16> for Prio2 {
    ...
    fn prepare_shares_to_prepare_message<M: IntoIterator<Item = Prio2PrepareShare>>(
        ...
        let verifier_shares: Vec<v2_server::VerificationMessage<Field64>> =
    ...
    fn aggregate<M: IntoIterator<Item = OutputShare<Field64>>>(
        &self,
        _agg_param: &Self::AggregationParam,
        out_shares: M,
    ) -> Result<AggregateShare<Field64>, VdafError> {
        let mut agg_share = AggregateShare(vec![Field64::zero(); self.input_len]);
...
impl Collector for Prio2 {
    fn unshard<M: IntoIterator<Item = AggregateShare<Field64>>>(
        &self,
        _agg_param: &Self::AggregationParam,
        agg_shares: M,
        _num_measurements: usize,
    ) -> Result<Vec<u64>, VdafError> {
        let mut agg = AggregateShare(vec![Field64::zero(); self.input_len]);
        for agg_share in agg_shares.into_iter() {
            agg.merge(&agg_share)?;
        }

        Ok(agg.0.into_iter().map(u64::from).collect())
    }
}

impl<'a> ParameterizedDecode<(&'a Prio2, usize)> for Share<Field64, 32> {
    fn decode_with_param(
        (prio2, agg_id): &(&'a Prio2, usize),
        bytes: &mut Cursor<&[u8]>,
    ) -> Result<Self, CodecError> {
...
mod tests {
    use super::*;
    use crate::vdaf::{equality_comparison_test, fieldvec_roundtrip_test, run_vdaf};
    use rand::prelude::*;
    ...
    fn roundtrip_output_share() {
        ...
        fieldvec_roundtrip_test::<Field64, Prio2, OutputShare<Field64>>(&vdaf, &(), 31);
    }

    #[test]
    fn roundtrip_aggregate_share() {
        let vdaf = Prio2::new(31).unwrap();
        fieldvec_roundtrip_test::<Field64, Prio2, AggregateShare<Field64>>(&vdaf, &(), 31);
    }
